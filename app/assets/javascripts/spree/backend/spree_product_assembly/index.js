  ...
  document.documentElement.addEventListener("turbo:load", (e) => {
    ...
    Spree.routes.available_admin_product_parts = function (productSlug) {
      return Spree.pathFor(
        "admin/products/" + productSlug + "/parts/available"
      );
    };
    ...
    searchResults.on("click", "a.add_product_part_link", function (event) {
      ...
      if (row.hasClass("with-variants")) {
        ...
        if (
          selectedVariantOption.text() === Spree.translations.user_selectable
        ) {
    ...
    return partsTable.on(
      "click",
      "a.remove_admin_product_part_link",
      function () {
